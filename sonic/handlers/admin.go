...
func CreatePlatform(ctx *gin.Context, licenseCreator LicenseCreator, fetchConfig FetchConfigFunction) gin.HandlerFunc {
	return func(context *gin.Context) {
		createPlatform(ctx, licenseCreator, fetchConfig)
		return
	}
}
func createPlatform(ctx *gin.Context, creator LicenseCreator, fetchConfig FetchConfigFunction) {
	// Get Opendax config
	...
	err = fetchConfig(sc.PeatioClient, opendaxConfig.Addr, platform.PID)
	if err != nil {
	...
	err = creator("finex", opendaxConfig, vaultService)
	if err != nil {
